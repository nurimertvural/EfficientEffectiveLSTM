...
classdef LSTM_IEKF < handle
   ...
   methods
       ...
       function [error_vec, kErr_vec] = train(obj)
            ...
            obj.chi_vec  = linspace( log2(obj.chi_min), log2(0.125) , obj.inst_num);
            ...
            for r = 1:obj.repeat
                ...
                for t=1:obj.Tx
                    ...
                    target_batch = obj.target_set(:,end_idx : end_idx + k - 1);
   ...
   methods(Access = private)
       ...
       function kErr = kStepError(obj, input_batch, target_batch, k)
           ...
           for t= 1:obj.batch_size + k - 1
            ...
            aug_ht   = [H_batchx(:, end- k + 1: end, :) ; ones( 1, k, obj.inst_num) ]; 
            kEst_aux = tanh( mtimesx( obj.temp_Wd , aug_ht  ) );
            
            loss_vectors = sum( ( target_batch' - reshape( kEst_aux, [k, obj.inst_num]) ).^2 );
            
            current_weights =  obj.weights .* exp(- loss_vectors');
            current_weights = current_weights / sum(current_weights);
            
            kEst  = sum(  mtimesx( kEst_aux(:, end, :), reshape(current_weights, [ 1, 1, obj.inst_num]) ), 3);
            kErr = (target_batch(:,end) - kEst).^2;
       ...
       function [dWdx, dWox, dWfx, dWix, dWzx]= forward_backward(obj,input_batch)
            ...
            pass_idx  = loss_vec < 4 * obj.chi_vec.^2; 
       ...
       function [Wdx, Wox, Wfx, Wix, Wzx, Phid, Pout] = update(obj, input_batch)
            ...
            theta_hid = theta_hid + G_hid .* reshape(obj.d_t - obj.d_hat_t_vec, 1, 1, 1, obj.inst_num);
            theta_out = theta_out + G_out .* (obj.d_t - obj.d_hat_t_vec);
            ...
            Phid = (Phid./G_hid - permute(intm_hid, [ 2 1 3 4])) .* G_hid ...
                    + obj.Q * eye(obj.n_h + obj.n_i) .* ones(1, 1, 4*obj.n_h, obj.inst_num );


            G_out( G_out == 0) = 1e-15;
            Pout = (Pout./G_out - permute(intm_out, [ 2 1 3]) ).*G_out ...
                   +  obj.Q .* ones( 1, 1, obj.inst_num );
